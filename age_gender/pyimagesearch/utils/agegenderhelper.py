...
class AgeGenderHelper:
    ...
    def buildOneOffMappings(self, le):
        classes = sorted(le.classes_, key=lambda x: int(str(x).split("_")[0]))
    ...
    def visualizeAge(self, age_probs):
        age_labels = [f"{lower}_{upper}" for lower, upper in self.ageBins[:-1]]
        plt.bar(age_labels, age_probs)
        plt.xlabel("Age Groups")
        plt.ylabel("Probability")
        plt.title("Age Class Probability Distribution")
        plt.xticks(rotation=45)
        plt.tight_layout()
        plt.show()

    def visualizeGender(self, gender_probs):
        labels = ['Male', 'Female']
        plt.bar(labels, gender_probs)
        plt.xlabel("Gender")
        plt.ylabel("Probability")
        plt.title("Gender Class Probability Distribution")
        plt.ylim(0, 1)
        plt.tight_layout()
        plt.show()
